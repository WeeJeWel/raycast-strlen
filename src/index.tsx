  ...
  return (
    ...
    >
      <List.Section title={`Input: ${input}`}>
        ...
        />
        {String(input).length !== stringLength(input) && <List.Item
          key={'length-corrected'}
          icon={{ source: Icon.Hashtag, tintColor: Color.Magenta }}
          title={'Length (Corrected)'}
          subtitle={{
            value: stringLength(input).toString(),
            tooltip: 'This string might contain astral symbols. This length reflects the string length with ansi escape codes ignored.',
          }}
        />}
        {String(input).length !== stringWidth(input) && <List.Item
          key={'width'}
          icon={{ source: Icon.Hashtag, tintColor: Color.Magenta }}
          title={'Width'}
          subtitle={{
            value: stringWidth(input).toString(),
            tooltip: 'The width of this string differs from its length due to special characters.',
          }}
        />}
      </List.Section>
    </List>
